...
.fluxf_time_unit_label <- function(ctx = NULL) {
  ...
  if (!is.null(ctx) && is.list(ctx)) {
    ...
    if (length(ctx) > 0L && all(vapply(ctx, is.list, logical(1)))) {
      for (i in seq_along(ctx)) {
        u <- tryCatch(.fluxf_time_unit_label(ctx[[i]]), error = function(e) NULL)
...
.fluxf_as_numeric_time <- function(x, name = "time", ctx = NULL) {
  if (inherits(x, "Date") || inherits(x, c("POSIXct", "POSIXt"))) {
    unit <- .fluxf_time_unit_label(ctx)
    stop(
      sprintf(
